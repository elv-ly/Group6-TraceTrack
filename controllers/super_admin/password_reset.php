...
try {
    ...
    // Prevent resetting super admin's password (optional but safer)
    ...
    // Allow super admin to change their own password, but not others' super admin passwords
    if ($user['role'] === 'super_admin' && $user_id !== $_SESSION['user_id']) {
        echo json_encode(["status" => false, "message" => "Cannot reset other super admin passwords"]);
        exit;
    }
